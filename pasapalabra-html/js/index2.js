...
function nextLetter(roscoLetters) {
    var everyLetter = roscoLetters.innerText;
    for (var i = 0; i < everyLetter.lenght; i++) {
        if (!isOver && !isAnwered) {
            checkAnswer(everyLetter[i]);
            markLetter(everyLetter[i]);
        }
    }
}
...
function updateLayout(rosco) {
    for (var i = 0; i < rosco.length; i++) {
        ...
        rosco[i].style.transform =
            "rotate(" +
            rotateAngle +
            "deg) translate(0, -190px) rotate(-" +
            rotateAngle +
            "deg)";
    }
}
